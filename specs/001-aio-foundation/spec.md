...
**Created**: 2026-05-10
**Status**: Draft
**Input**: User description: "Scaffold a Bicep deployment for an all-in-one Azure AI foundation (AI Foundry hub + project, Azure OpenAI with frontier text + image models, Anthropic Claude models via Foundry, Storage, Key Vault, dependencies). Must deploy via GitHub Actions (OIDC) and a local script. Nothing personal/private may live in source. Modular templates, central naming/tagging, and reusable functions."
...
OpenAI account with the configured model deployments, the Foundry resource that
hosts Anthropic Claude model deployments, Storage, Key Vault, Log Analytics,
Application Insights, and dependencies (managed identity, RBAC role
assignments, diagnostic settings). The workflow logs never reveal the tenant
ID, subscription ID, or any keys.
...
`rg-aio-dev-cc` and that the Foundry project lists the configured OpenAI and
...
1. **Given** a freshly created Azure subscription with no prior deployments
   ...
   **Then** the workflow completes successfully, the resource group
   ...
   one Foundry project, one Azure OpenAI account, one Foundry-hosted Claude
   account, a Storage account, a Key Vault, a Log Analytics workspace, and
   an Application Insights instance — each tagged per the constitution.
...
The operator wants to add a new OpenAI or Claude model deployment to `dev`.
...
- Claude in Foundry requires a Marketplace subscription on the Azure
  account → if missing, the deployment must fail with an actionable error
  pointing to the Marketplace acceptance step.
...
- **FR-005**: Reusable resource modules MUST live under
  ...
  `foundry-project`, `openai-account`, `openai-deployment`,
  `foundry-claude-account`, `foundry-claude-deployment`, `storage`,
  `key-vault`, `log-analytics`, `app-insights`, `role-assignment`.
...
- **FR-019**: The deployment MUST provision one Foundry-hosted account
  capable of running Anthropic Claude models and create Claude model
  deployments listed in the parameter file (e.g.,
...
- **FR-022**: A user-assigned managed identity MUST be created and
  granted least-privilege RBAC on the OpenAI account, the Foundry
  account, the Storage account, and the Key Vault.
...
- **SC-003**: Adding one new OpenAI model deployment requires editing
  **exactly one file** (`main.dev.bicepparam`) and **zero lines** of
  Bicep module code.
...
- Anthropic Claude models in Foundry require a one-time Azure
  Marketplace acceptance per subscription; the operator will perform
  this manual step (it is documented in `quickstart.md`).
- GPT-image-1 / 1.5 require Microsoft-managed access approval; they are
  parameterized but NOT enabled by default.
- The default region for `dev` is `eastus2` because it is the only
  region currently hosting both the latest Azure OpenAI frontier text
  models and Anthropic Claude in Foundry; this is recorded and may be
  overridden per parameter file.
